...
const WorksManagement = () => {
  ...
  useEffect(() => {
    novelApi.getNovelDetail({ id: novelId }).then(res => {
      ...
      if (data.tags && Array.isArray(data.tags)) {
        const statusTag = data.tags.find((t: any) => t.type === TAG_TYPE_STATUS);
        if (statusTag && statusTag.name) {
          status = statusTag.name || status;
        }

        const channelTag = data.tags.find((t: any) => t.type === TAG_TYPE_CHANNEL);
        if (channelTag && channelTag.name) {
          channel = channelTag.name || channel;
        }

        const categoryTags = data.tags.filter((t: any) => t.type === TAG_TYPE_CATEGORY);
        if (categoryTags.length > 0) {
          categories = categoryTags.map((t: any) => {
            return t.name;
          });
        }
      }
  ...
  // 处理作品信息中普通字段的修改
  const handleWorkInfoChange = (e: any) => {
    const { name, value } = e.target;
    setWorkInfo((prev: any) => ({
      ...prev,
      [name]: value
    }));
  };
  ...
  // 处理分类多选
  const handleCategoryChange = (catValue: any) => {
    setWorkInfo((prev: any) => {
      ...
      if (current.includes(catValue)) {
        ...
        return { ...prev, categories: current.filter((c: any) => c !== catValue) };
      } else {
  ...
  // 处理章节表单输入
  const handleChapterFormChange = (e: any) => {
    const { name, value } = e.target;
  ...
  const handleSaveChapter = async () => {
    ...
    if (editingId !== null) {
      const originalChapter = chapters.find((ch: any) => ch.id === editingId);
  ...
  const handleDeleteChapter = async (chapterId: any) => {
    ...
    // 被删除章节的内容长度
    const chapterToDelete = chapters.find((ch: any) => ch.id === chapterId);
    if (!chapterToDelete) {
      alert('章节不存在');
      return;
    }
      ...
      <section className={styles.section}>
        ...
        {chapters.length > 0 ? (
          <ul className={styles.chapterList}>
            {chapters.map((chapter: any) => <li key={chapter.chapter_number} className={styles.chapterItem}>
