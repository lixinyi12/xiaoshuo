...
const NovelRead = () => {
  ...
  // 状态管理
  const [currentChapter, setCurrentChapter] = useState<Chapter | null>(null); // 当前章节数据
  const [chapterList, setChapterList] = useState<Chapter[]>([]); // 章节列表
  const [fontSize, setFontSize] = useState(16); // 字体大小
  const [isDarkMode, setIsDarkMode] = useState(false); // 是否夜间模式
  const [bookmarks, setBookmarks] = useState<number[]>([]); // 书签列表
  ...
  // 获取特定章节内容
  const fetchChapterContent = async (targetChapterNumber: any) => {
    try {
      setLoading(true);
      ...
      if (chapters.length > 0) {
        ...
        if (chapterNumber) {
          const targetChapter = chapters.find(
            (ch: any) => ch.chapter_number.toString() === chapterNumber.toString()
          );
          if (targetChapter) {
  ...
  const goToNextChapter = async () => {
    ...
    // 检查下一章是否存在
    const nextChapter = chapterList.find((ch: any) => parseInt(ch.chapter_number) === nextChapterNum
    );
  ...
  const goToPrevChapter = async () => {
    ...
    if (prevChapterNum >= 1) {
      // 检查上一章是否存在
      const prevChapter = chapterList.find((ch: any) => parseInt(ch.chapter_number) === prevChapterNum
      );
  ...
  // 点击章节列表中的章节
  const handleChapterClick = async (chapterNumber: any) => {
    await fetchChapterContent(chapterNumber);
    setShowChapterList(false);
  };

  // 调整字体大小
  const adjustFontSize = (delta: any) => {
    ...
    <div className={`${styles.novelReader} ${isDarkMode ? styles.darkMode : ''}`}>
      ...
      {showChapterList && (
        <div className={styles.chapterList}>
          ...
          <ul>
            {chapterList.map((chapter: Chapter) => <li
              key={chapter.id}
      ...
      <footer className={styles.readerFooter}>
        ...
        <button
          className={styles.navButton}
          disabled={!currentChapter || !chapterList.find((ch: any) => parseInt(ch.chapter_number) === parseInt(currentChapter.chapterNumber) + 1
          )}
